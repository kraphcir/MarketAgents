...
SWARM_MODELS = {
    # Active Swarm Models - 3 Model Configuration
    ...
    # [i] See all 200+ models at: https://openrouter.ai/docs
    # [i] Any model from openrouter_model.py can be used here!
}
...
MODEL_TIMEOUT = 120  #  Increased to 120s for more reliability
...
class SwarmAgent:
    """ Swarm Agent for multi-model consensus"""
    ...
    def __init__(self, custom_models: Optional[Dict] = None):
        ...
        cprint("\n" + "="*60, "cyan")
        cprint(" Swarm Agent Initialized [*]", "cyan", attrs=['bold'])
        cprint("="*60, "cyan")
        cprint(f"\n[AI] Active Models in Swarm: {len(self.active_models)}", "green")
        ...
        for provider, (enabled, model_type, model_name) in self.models_config.items():
            ...
            try:
                ...
                if model:
                    ...
                    cprint(f"[!] Could not initialize {provider}: {model_name}", "yellow")
            except Exception as e:
                cprint(f"[X] Error initializing {provider}: {e}", "red")
    ...
    def _query_single_model(self, provider: str, model_info: Dict, prompt: str,
        ...
        try:
            ...
            elapsed = time.time() - start_time
            cprint(f"[X] Error querying {provider}: {e}", "red")
    ...
    def query(self, prompt: str, system_prompt: Optional[str] = None) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=len(self.active_models)) as executor:
            ...
            try:
                for future in as_completed(futures, timeout=MODEL_TIMEOUT + 10):
                    ...
                    try:
                        ...
                        if response["success"]:
                            ...
                            cprint(f"   [X] {provider} failed: {response['error']}", "red")

                    except TimeoutError:
                        cprint(f"   [T] {provider} timed out (>{MODEL_TIMEOUT}s) - skipping", "yellow")
                        ...
                        cprint(f"   [X] {provider} error: {str(e)}", "red")
                        all_responses[provider] = {
                            "provider": provider,
                            "model": "error",
                            "response": None,
                ...
                cprint(f"\n[T] Overall timeout reached - some models didn't respond", "yellow")
                cprint(f"[!] {str(timeout_error)}", "yellow")
                # Mark any remaining futures as timed out
                for future, provider in futures.items():
                    if provider not in all_responses:
                        cprint(f"   [T] {provider} never responded - marking as timeout", "red")
                        all_responses[provider] = {
                            "provider": provider,
                            "model": "timeout",
                            "response": None,
                ...
                # [*] Swarm - Don't raise, continue with partial results
    ...
    def _generate_consensus_review(self, responses: Dict[str, Dict], original_prompt: str) -> Tuple[str, Dict]:
        ...
        try:
            ...
            cprint(f"\n[AI] Generating consensus summary with {model_name}...", "magenta")
            ...
            cprint(f"[X] Error generating consensus review: {e}", "red")
    ...
    def _save_results(self, result: Dict):
        ...
        cprint(f"\n[S] Results saved to: {filename.relative_to(Path(project_root))}", "blue")
    ...
    def _print_summary(self, result: Dict):
        ...
        if "consensus_summary" in result:
            cprint("\n[AI] AI CONSENSUS SUMMARY:", "magenta", attrs=['bold'])
...
def main():
    ...
    cprint("\n" + "="*60, "cyan")
    cprint("[*] Swarm's Swarm Agent [*]", "cyan", attrs=['bold'])
    cprint("="*60, "cyan")
    ...
    if not prompt:
        cprint("[X] No prompt provided. Exiting.", "red")
        return
    ...
    cprint("\n" + "="*60, "cyan")
    cprint("[L] AI RESPONSES", "cyan", attrs=['bold'])
    cprint("="*60, "cyan")
    ...
    for provider, data in result["responses"].items():
        if data["success"]:
            ...
            if ai_label:
                cprint(f"\n[AI] {ai_label} ({provider.upper()}):", "yellow", attrs=['bold'])
            else:
                cprint(f"\n[AI] {provider.upper()}:", "yellow", attrs=['bold'])
            ...
            cprint(f"[T]  Response time: {data['response_time']}s", "cyan")
        else:
            cprint(f"\n[X] {provider.upper()}: Failed - {data['error']}", "red")
