...
CONSENSUS_PICKS_CSV = os.path.join(DATA_FOLDER, "consensus_picks.csv")  # [*] Agent - Top consensus picks only
...
class PolymarketAgent:
    ...
    def _load_predictions(self):
        ...
        # Create new DataFrame with required columns
        # [*] Agent - Link column at END for clickable CSVs in Excel/Numbers
        return pd.DataFrame(columns=[
    ...
    def should_ignore_market(self, title):
        """[*] Agent - Check if market should be ignored based on category keywords
    ...
    def on_ws_message(self, ws, message):
        """[*] Agent - Handle incoming WebSocket messages"""
        try:
            ...
            if isinstance(data, dict):
                ...
                if data.get('type') == 'subscribed':
                    cprint("[+] Agent WebSocket subscribed successfully to live trades!", "green")
                ...
                if topic == 'activity' and msg_type == 'orders_matched':
                    ...
                    # [*] Agent - Check if we should ignore this market category
                    ...
                    if usd_amount >= MIN_TRADE_SIZE_USD and not self.is_near_resolution(price):
                        ...
                        # [*] AGENT - Process this trade immediately
                        trade_data = {
            ...
            cprint(f"[!] Agent - Error processing WebSocket message: {e}", "yellow")

    def on_ws_error(self, ws, error):
        """[*] Agent - Handle WebSocket errors"""
        cprint(f"[X] Agent WebSocket Error: {error}", "red")

    def on_ws_close(self, ws, close_status_code, close_msg):
        """[*] Agent - Handle WebSocket close"""
        self.ws_connected = False
        cprint(f"\n[~] Agent WebSocket connection closed: {close_status_code} - {close_msg}", "yellow")
        cprint("Reconnecting in 5 seconds...", "cyan")
        time.sleep(5)
        self.connect_websocket()

    def on_ws_open(self, ws):
        """[*] Agent - Handle WebSocket open - send subscription"""
        cprint("[~] Agent WebSocket connected!", "green")
        ...
        cprint(f"[*] Agent sending subscription for live trades...", "cyan")
        ws.send(json.dumps(subscription_msg))

        # Set connected flag immediately after sending subscription
        self.ws_connected = True
        cprint("[+] Agent subscription sent! Waiting for trades...", "green")
    ...
    def connect_websocket(self):
        """[*] Agent - Connect to Polymarket WebSocket"""
        cprint(f"[>] Agent connecting to {WEBSOCKET_URL}...", "cyan")
        ...
        cprint("[+] Agent WebSocket thread started!", "green")

    def fetch_historical_trades(self, hours_back=None):
        """[*] Agent - Fetch historical trades from Polymarket API on startup
        ...
        try:
            cprint(f"\n[*] Agent fetching historical trades (last {hours_back}h)...", "yellow")
            ...
            cprint(f"[X] Error fetching historical trades: {e}", "red")
            return []
        ...
        for market_id, trade in unique_markets.items():
            try:
                ...
                if market_id in self.markets_df['market_id'].values:
                    # [*] Agent - UPDATE existing market with new trade data (fresh odds!)
    ...
    def display_recent_markets(self):
        ...
        for idx, row in recent.iterrows():
            ...
            cprint(f"[L] https://polymarket.com/event/{row['event_slug']}", "cyan")
    ...
    def get_ai_predictions(self):
        ...
        cprint("\n" + "="*80, "magenta")
        cprint(f"[AI] AI Analysis - Analyzing {len(markets_to_analyze)} markets", "magenta", attrs=['bold'])
        ...
        # [*] Agent - Conditionally include price info based on config
...
Provide predictions for each market in the specified format."""
        ...
        if USE_SWARM_MODE and self.swarm:
            ...
            cprint("[*] Agent sending prompts to swarm...", "cyan")
            ...
            # [*] Agent - Run final consensus AI to pick top 3 markets
            ...
            # [*] Agent - Mark analyzed markets with timestamp
    ...
    def _mark_markets_analyzed(self, markets, analysis_timestamp):
        """[*] Agent - Mark markets as analyzed with timestamp
    ...
    def _save_swarm_predictions(self, analysis_run_id, analysis_timestamp, markets, swarm_result):
        """[*] Agent - Save swarm predictions to CSV database (one row per market)
        ...
        try:
                ...
                for line in lines:
                    ...
                    if 'MARKET' in line_upper and ':' in line:
                        try:
                            ...
                            # [*] Agent - Validate market number is within range
            ...
            for market_num, predictions in market_predictions.items():
                ...
                if 1 <= market_num <= len(markets_list):
                    ...
                    record = {
                        ...
                        'market_link': market_link  # [*] Agent - Link at end for clickable CSVs
                    }
            ...
            if new_records:
                ...
                cprint(f"[!] No structured predictions found to save", "yellow")
    ...
    def _calculate_polymarket_consensus(self, swarm_result, markets_df):
        """
        [*] Agent - Calculate consensus from individual swarm responses for Polymarket predictions
        ...
        try:
            ...
            cprint(f"[X] Error calculating polymarket consensus: {e}", "red")
            import traceback
            traceback.print_exc()
    ...
    def _get_top_consensus_picks(self, swarm_result, markets_df):
        """
        [*] Agent - Use consensus AI to identify top 3 markets with strongest agreement
        ...
        try:
            cprint("\n" + "="*80, "yellow")
            cprint("[AI] Running Consensus AI to identify top 3 picks...", "yellow", attrs=['bold'])
            cprint("="*80 + "\n", "yellow")
            ...
            cprint(f"[#1] TOP {TOP_MARKETS_COUNT} CONSENSUS PICKS - AGENT AI RECOMMENDATION", "white", "on_blue", attrs=['bold'])
            ...
            # [*] Agent - Save consensus picks to dedicated CSV
    ...
    def _save_consensus_picks_to_csv(self, consensus_response, markets_df):
        """
        [*] Agent - Save top consensus picks to dedicated CSV (append-only)
        ...
        try:
            ...
            # Convert to records for CSV ([*] Agent - link at END for clickable CSVs)
            records = []
            for pick in picks:
                record = {
                    'timestamp': timestamp,
            ...
            # Load or create consensus picks CSV ([*] Agent - link at END for clickable CSVs)
    ...
    def status_display_loop(self):
        """[*] Agent - Display status updates every 30 seconds"""
        ...
        while True:
            try:
                ...
                # [*] Agent - Count markets with FRESH TRADES that are also ELIGIBLE
                now = datetime.now()
                ...
                cprint(f"\n{'='*60}", "cyan")
                cprint(f"[#] Agent Status @ {datetime.now().strftime('%H:%M:%S')}", "cyan", attrs=['bold'])
                cprint(f"{'='*60}", "cyan")
                cprint(f"   WebSocket Connected: {'[+] YES' if self.ws_connected else '[X] NO'}", "green" if self.ws_connected else "red")
    ...
    def analysis_cycle(self):
        ...
        cprint("\n" + "="*80, "magenta")
        cprint("[AI] ANALYSIS CYCLE CHECK", "magenta", attrs=['bold'])
        ...
        # [*] Agent - Skip if no markets exist yet
        if total_markets == 0:
        ...
        # [*] Agent - Count markets with FRESH TRADES that are also ELIGIBLE for re-analysis
        now = datetime.now()
        cutoff_time = now - timedelta(hours=REANALYSIS_HOURS)
        ...
        if should_analyze:
            ...
            # [*] Agent - Update analysis run timestamp
            ...
            cprint(f"[T] Next run will only count markets with fresh trades after {datetime.now().strftime('%H:%M:%S')}", "cyan")
        else:
    ...
    def analysis_loop(self):
        """[*] Agent - Continuously check for new markets to analyze (runs immediately!)"""
        cprint("\n[AI] ANALYSIS THREAD STARTED", "magenta", attrs=['bold'])
        cprint(f"[AI] Running first analysis NOW, then checking every {ANALYSIS_CHECK_INTERVAL_SECONDS} seconds\n", "magenta")

        # [*] Agent - Run first analysis IMMEDIATELY (no waiting!)
        cprint("[>] Agent running first analysis immediately...\n", "yellow", attrs=['bold'])
...
def main():
    """[*] Agent Main - WebSocket real-time data + AI analysis threads"""
    cprint("\n" + "="*80, "cyan")
    cprint("[*] Agent's Polymarket Agent - WebSocket Edition!", "cyan", attrs=['bold'])
    cprint("="*80, "cyan")
    ...
    cprint(f"[X] Filtering out crypto/Bitcoin markets ({len(IGNORE_CRYPTO_KEYWORDS)} keywords)", "red")
    cprint(f"[X] Filtering out sports markets ({len(IGNORE_SPORTS_KEYWORDS)} keywords)", "red")
    cprint(f"[H] Lookback period: {LOOKBACK_HOURS} hours (fetches historical data on startup)", "yellow")
    cprint("", "yellow")
    cprint("[~] REAL-TIME WebSocket MODE:", "green", attrs=['bold'])
    cprint(f"   [W] WebSocket: {WEBSOCKET_URL}", "cyan")
    cprint(f"   [#] Status Display: Every 30s - Shows collection stats", "cyan")
    cprint(f"   [AI] Analysis Thread: Every {ANALYSIS_CHECK_INTERVAL_SECONDS}s - Checks for new markets", "magenta")
    cprint(f"   [>] AI Analysis triggers when {NEW_MARKETS_FOR_ANALYSIS} new markets collected", "yellow")
    cprint("", "yellow")
    cprint(f"[AI] AI Mode: {'SWARM (6 models)' if USE_SWARM_MODE else 'Single Model'}", "yellow")
    ...
    # [*] Agent - Fetch historical trades on startup to populate database
    cprint("\n" + "="*80, "yellow")
    cprint(f"[H] Agent fetching historical data from last {LOOKBACK_HOURS} hours...", "yellow", attrs=['bold'])
    cprint("="*80, "yellow")

    historical_trades = agent.fetch_historical_trades()
    if historical_trades:
        cprint(f"\n[P] Processing {len(historical_trades)} historical trades...", "cyan")
        agent.process_trades(historical_trades)
        cprint(f"[+] Database populated with {len(agent.markets_df)} markets", "green")
    else:
        cprint("[!] No historical trades found - will start fresh from WebSocket", "yellow")
    ...
    # Start threads
    try:
        cprint("[>] Agent starting threads...\n", "green", attrs=['bold'])
        status_thread.start()
        analysis_thread.start()

        # Keep main thread alive
        cprint("[+] Agent WebSocket + AI running! Press Ctrl+C to stop.\n", "green", attrs=['bold'])
        while True:
            time.sleep(1)

    except KeyboardInterrupt:
        cprint("\n\n" + "="*80, "yellow")
        cprint("[!] Agent Polymarket Agent stopped by user", "yellow", attrs=['bold'])
        cprint("="*80 + "\n", "yellow")
        sys.exit(0)
