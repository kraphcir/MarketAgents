...
class GroqModel(BaseModel):
    ...
    def __init__(self, api_key: str, model_name: str = "qwen/qwen3-32b", **kwargs):
        try:
            cprint(f"\n[*] Groq Model Initialization", "cyan")
            ...
            cprint(f"\n[X] Error in Groq model initialization", "red")
    ...
    def initialize_client(self, **kwargs) -> None:
        ...
        try:
            ...
            if self.model_name not in api_models:
                cprint(f"  - [!] Requested model not found in API", "yellow")
            ...
            cprint(f"\n[X] Failed to initialize Groq client", "red")
            cprint(f"  - Error type: {type(e).__name__}", "red")
            cprint(f"  - Error message: {str(e)}", "red")
    ...
    def generate_response(self, system_prompt, user_content, temperature=0.7, max_tokens=None):
        ...
        try:
            ...
            if "413" in error_str or "rate_limit_exceeded" in error_str:
                cprint(f"[!] Groq rate limit exceeded (request too large)", "yellow")
            ...
            # Log other errors
            cprint(f"[X] Groq error: {error_str}", "red")
            return None
