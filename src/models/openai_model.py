...
class OpenAIModel(BaseModel):
    ...
    def initialize_client(self, **kwargs) -> None:
        ...
        try:
            ...
            cprint(f"[X] Failed to initialize OpenAI model: {str(e)}", "red")
            self.client = None
    ...
    def generate_response(self, system_prompt, user_content, **kwargs):
        ...
        try:
            ...
            if self.model_name.startswith(('gpt-5', 'o1')):
                try:
                    ...
                    try:
                        ...
                        cprint(f"[X] Direct Responses HTTP fallback failed: {repr(http_e)}", "red")
                        cprint("[!] Falling back to Chat Completions", "yellow")
            ...
            try:
                ...
                cprint(f"[*] debug: finish_reason={finish_reason}", "cyan")
            except Exception:
                pass
            ...
            if not content_text:
                cprint("[!] OpenAI returned empty content", "yellow")

            # If still empty, do a single simplified retry (matches other GPT handling)
            if not content_text:
                cprint("[*] Retrying once with simplified prompt format (fallback)", "yellow")
                retry_messages = [
                    {
                        "role": "user",
            ...
            if not content_text and hasattr(self.client, 'responses'):
                try:
                    cprint("[*] Fallback: trying Responses API for text output", "yellow")
                    content_str = f"Instructions: {system_prompt}\n\nInput: {user_content}"
                    ...
                    cprint(f"[X] Responses API fallback error: {repr(fallback_e)}", "red")

            # Final safety net: fallback to a stable chat model if nothing came back
            if (not content_text) and self.model_name == 'gpt-5':
                try:
                    cprint("[*] Fallback: retrying with gpt-4o to avoid empty content", "yellow")
                    fallback_messages = [
                    ...
                    cprint(f"[X] Fallback to gpt-4o failed: {repr(fb_e)}", "red")
            ...
            cprint(f"[X] OpenAI generation error: {repr(e)}", "red")
            try:
