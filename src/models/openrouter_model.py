...
class OpenRouterModel(BaseModel):
    ...
    def __init__(self, api_key: str, model_name: str = "google/gemini-2.5-flash", **kwargs):
        try:
            cprint(f"\n[*] OpenRouter Model Initialization", "cyan")
            ...
            if model_name not in self.AVAILABLE_MODELS:
                cprint(f"  - [!] Model not in predefined list (will still try to use it)", "yellow")
            ...
            cprint(f"\n[X] Error in OpenRouter model initialization", "red")
    ...
    def initialize_client(self, **kwargs) -> None:
        ...
        try:
            ...
            cprint(f"\n[X] Failed to initialize OpenRouter client", "red")
            cprint(f"  - Error type: {type(e).__name__}", "red")
            cprint(f"  - Error message: {str(e)}", "red")
    ...
    def generate_response(self, system_prompt, user_content, temperature=0.7, max_tokens=None):
        ...
        try:
            ...
            if "429" in error_str or "rate_limit" in error_str:
                cprint(f"[!] OpenRouter rate limit exceeded", "yellow")
            ...
            if "402" in error_str or "insufficient" in error_str:
                cprint(f"[!] OpenRouter credits insufficient", "yellow")
                cprint(f"   Model: {self.model_name}", "yellow")
            ...
            # Log other errors
            cprint(f"[X] OpenRouter error: {error_str}", "red")
            return None
