...
class OllamaModel(BaseModel):
            ...
            if response.status_code == 200:
                ...
                if models:
                    ...
                    if self.model_name not in model_names:
                        cprint(f"[!] Model {self.model_name} not found! Please run:", "yellow")
                        cprint(f"   ollama pull {self.model_name}", "yellow")
                else:
                    cprint("[!] No models found! Please pull the model:", "yellow")
                    cprint(f"   ollama pull {self.model_name}", "yellow")
            else:
                cprint(f"[!] Ollama API returned status code: {response.status_code}", "yellow")
                raise ConnectionError(f"Ollama API returned status code: {response.status_code}")
        except requests.exceptions.ConnectionError:
            cprint("[X] Could not connect to Ollama API - is the server running?", "red")
            cprint("[*] Start the server with: ollama serve", "yellow")
            raise
        except Exception as e:
            cprint(f"[X] Could not connect to Ollama API: {str(e)}", "red")
    ...
    def generate_response(self, system_prompt, user_content, temperature=0.7, max_tokens=None, **kwargs):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                cprint(f"[X] Ollama API error: {response.status_code}", "red")
                cprint(f"Response: {response.text}", "red")
                raise Exception(f"Ollama API error: {response.status_code}")

        except Exception as e:
            cprint(f"[X] Error generating response: {str(e)}", "red")
    ...
    def get_model_parameters(self, model_name=None):
        ...
        try:
            ...
            cprint(f"[X] Error getting model parameters: {str(e)}", "red")
            return None
